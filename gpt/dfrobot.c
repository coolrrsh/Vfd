...
static uint32_t SPI_SPEED = 1000000; // 1 MHz default
 ...
 *********************************************************************/
int main() {
    printf("DFRobot_CH432T Full Driver C Example\n");

    // Create handle for port 0, 115200, 8N1
    DFRobot_CH432T *ch432 = CH432T_create(CH432T_PORT_1, 115200, DATABITS_8, PARITY_NONE, STOPBITS_ONE);
    ...
    // test write
    const char *test_str = "Hello CH432T\n";
    CH432T_write(ch432, (const uint8_t*)test_str, strlen(test_str));

    // test read (non-blocking for ~1 second)
    uint8_t read_buf[100];
    memset(read_buf, 0, sizeof(read_buf));
    int len = CH432T_read(ch432, read_buf, 10, 1000);
    if (len > 0) {
