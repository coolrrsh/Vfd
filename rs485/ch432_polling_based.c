...
// CH432T driver instance - simplified for polling
typedef struct {
...
static bool data_available(DFRobot_CH432T *handle) {
    uint8_t lsr = read_register(handle, CH432T_LSR_REG);
    return (lsr & CH432T_LSR_DR_BIT);
}

static bool tx_ready(DFRobot_CH432T *handle) {
    uint8_t lsr = read_register(handle, CH432T_LSR_REG);
    return (lsr & CH432T_LSR_THRE_BIT);
}
...
static void ch432t_reconfigure_port(DFRobot_CH432T *handle) {
    ...
    write_register_single(handle, CH432T_IER_REG, 0);
...
int CH432T_read(DFRobot_CH432T *handle, uint8_t *buf, size_t size, unsigned int timeout_ms) {
    ...
    while (read_count < size) {
        ...
        if (data_available(handle)) {
            buf[read_count++] = read_register(handle, CH432T_RBR_REG);
            continue;
        }

        // Check timeout if no data available
        if (timeout_ms == 0) {
            break; // Non-blocking mode
        }

        clock_gettime(CLOCK_MONOTONIC, &now);
        unsigned long elapsed_ms = (now.tv_sec - start.tv_sec) * 1000UL + 
...
int main() {
    ...
    while(1) {
        ...
        // Read response (using polling-based read with 1 second timeout)
